      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildParagraph(
                'Flutter 是 Google 的开源 UI 工具包，用于从单一代码库构建原生编译的多平台应用。本应用基于 Flutter 框架构建。'),
            _buildDivider(),
            _buildTitle(context, 'Dio'),
            _buildParagraph('许可证：MIT License'),
            _buildParagraph(
                'Dio 是一个强大的 HTTP 网络请求库，支持拦截器、全局配置、FormData、请求取消、文件下载等功能。本应用使用 Dio 进行网络通信。'),
            _buildDivider(),
            _buildTitle(context, 'Provider'),
            _buildParagraph('许可证：MIT License'),
            _buildParagraph(
                'Provider 是 Flutter 的状态管理库，提供了简单且高效的方式在 Widget 树中共享数据。本应用使用 Provider 进行状态管理。'),
            _buildDivider(),
            _buildTitle(context, 'CachedNetworkImage'),
            _buildParagraph('许可证：MIT License'),
            _buildParagraph(
                'Cached Network Image 用于缓存网络图片并提供加载过程中的占位符。本应用使用它来优化图片加载体验。'),
            _buildDivider(),
            _buildTitle(context, 'share_plus'),
            _buildParagraph('许可证：BSD 3-Clause License'),
            _buildParagraph(
                'share_plus 是 Flutter 的分享插件，支持通过系统分享菜单将文本、链接和文件分享到其他应用。本应用使用它来实现内容分享功能。'),
